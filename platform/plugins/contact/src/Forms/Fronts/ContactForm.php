    ...
    public function setup(): void
    {
        ...
        $this
            ...
            ->add(
                ...
                'hidden',
                TextFieldOption::make()
                    ->value(Arr::get($data, 'display_custom_fields'))
            )
            ->addRowWrapper('form_wrapper', function (self $form) use ($displayFields, $mandatoryFields): void {
                ...
                try {
                    $displayCustomFields = Arr::get($data, 'display_custom_fields');
                    if ($displayCustomFields) {
                        $selectedCustomFieldIds = is_array($displayCustomFields) 
                            ? $displayCustomFields 
                            : array_filter(explode(',', (string) $displayCustomFields));
                    }
                } catch (Throwable) {
                    $selectedCustomFieldIds = [];
                }
                
                // Only show custom fields if they are explicitly selected
                if (!empty($selectedCustomFieldIds)) {
                    ...
                    // Return empty Eloquent Collection to maintain type consistency
                    $customFields = CustomField::query()->whereRaw('1 = 0')->get();
                }
            ...
            ->when(false, function (self $form): void {
                $form->add(
