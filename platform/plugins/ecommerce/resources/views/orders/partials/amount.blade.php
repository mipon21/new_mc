...
<div class="mt-2 p-2">
    ...
    @if (EcommerceHelper::isTaxEnabled())
        <div class="row">
            <div class="col-6">
                <p>{{ __('Product Tax') }} @if (Cart::instance('cart')->rawTax())
    ...
    @if (!empty($shipping) && Arr::get($sessionCheckoutData, 'is_available_shipping', true))
        ...
        @if (EcommerceHelper::isTaxEnabled() && $shippingAmount > 0)
            <div class="row">
                <div class="col-6">
                    <p>{{ trans('plugins/ecommerce::products.form.shipping_tax') }} (19%):</p>
                </div>
                <div class="col-6 float-end">
                    <p class="price-text shipping-tax-price-text">{{ format_price(Cart::instance('cart')->rawShippingTax($shippingAmount)) }}</p>
                </div>
            </div>
        @endif
    @endif
    ...
    <div class="row">
        ...
        <div class="col-6 float-end">
            <p class="total-text raw-total-text" data-price="{{ format_price($rawTotal, null, true) }}">
                {{ format_price($orderAmount + Cart::instance('cart')->rawShippingTax($shippingAmount)) }}
            </p>
        </div>
    </div>
</div>
