...
<x-core::form.fieldset class="stock-status-wrapper" @style(['display: none' => old('with_storehouse_management', $product ? $product->with_storehouse_management : $originalProduct->with_storehouse_management ?? 0) == 1])>
    ...
    @foreach (Botble\Ecommerce\Enums\StockStatusEnum::labels() as $status => $label)
        <x-core::form.checkbox
            ...
            :inline="true"
            class="stock-status-radio"
            data-status="{{ $status }}"
        />
    @endforeach

    <div class="shipping-weeks-wrapper" @style(['display: none' => old('stock_status', $product ? $product->stock_status : 'in_stock') != 'in_stock_with_shipping'])>
        <x-core::form.text-input
            :label="trans('plugins/ecommerce::products.form.shipping_weeks')"
            name="shipping_weeks"
            :value="old('shipping_weeks', $product ? $product->shipping_weeks : '')"
            :placeholder="trans('plugins/ecommerce::products.form.shipping_weeks_placeholder')"
        />
    </div>
</x-core::form.fieldset>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        const stockStatusRadios = document.querySelectorAll('.stock-status-radio');
        const shippingWeeksWrapper = document.querySelector('.shipping-weeks-wrapper');

        stockStatusRadios.forEach(radio => {
            radio.addEventListener('change', function() {
                if (this.value === 'in_stock_with_shipping') {
                    shippingWeeksWrapper.style.display = 'block';
                } else {
                    shippingWeeksWrapper.style.display = 'none';
                }
            });
        });
    });
