 ...
 * @method static ShippingRuleTypeEnum BASED_ON_QUANTITY()
...
class ShippingRuleTypeEnum extends Enum
{
    ...
    public function toHtml(): HtmlString|string
    {
        return match ($this->value) {
            ...
            self::BASED_ON_WEIGHT => Html::tag('span', self::BASED_ON_WEIGHT()->label(), ['class' => 'text-info'])
                ->toHtml(),
            self::BASED_ON_QUANTITY => Html::tag('span', self::BASED_ON_QUANTITY()->label(), ['class' => 'text-warning'])
                ->toHtml(),
    ...
    public static function getLabel(?string $value): ?string
    {
        ...
        if ($value == self::BASED_ON_QUANTITY) {
            $replace['unit'] = self::BASED_ON_QUANTITY()->toUnit();
        }
    ...
    public function toUnit(): ?string
    {
        return match ($this->value) {
            ...
            self::BASED_ON_QUANTITY => trans('plugins/ecommerce::shipping.quantity_unit'),
            default => '',
        };
    }
    ...
    public function toUnitText(mixed $value): mixed
    {
        return match ($this->value) {
            ...
            self::BASED_ON_QUANTITY => number_format($value) . ' ' . $this->toUnit(),
            default => $value,
        };
    }
    ...
    {
        return match ($this->value) {
            self::BASED_ON_PRICE, self::BASED_ON_WEIGHT, self::BASED_ON_QUANTITY => true,
            default => false,
        };
    }
