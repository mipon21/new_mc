...
class InvoiceHelper
{
        ...
        if (is_plugin_active('payment')) {
            ...
            if ($payment) {
                $invoiceData = array_merge($invoiceData, [
                    ...
                    'status' => $payment->status == PaymentStatusEnum::COMPLETED ? InvoiceStatusEnum::COMPLETED : InvoiceStatusEnum::PENDING,
    ...
    protected function getDataForInvoiceTemplate(Invoice $invoice): array
    {
        ...
        $language = LanguageFacade::getCurrentLocale();
        $locale = is_array($language) && isset($language['locale']) ? $language['locale'] : 'en';
        $isRtl = is_array($language) && isset($language['is_rtl']) ? $language['is_rtl'] : false;
    ...
    public function getLanguageSupport(): string
    {
        ...
        $currentLocale = LanguageFacade::getCurrentLocale();
        
        // Map locale to language support
        $languageMap = [
            'en' => 'default',
            'de' => 'default', // German uses default (Latin) support
        ];

        return $languageMap[$currentLocale] ?? 'default';
    }
