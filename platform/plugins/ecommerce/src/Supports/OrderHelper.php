...
class OrderHelper
{
    ...
    public function processOrderInCheckout(
        ...
        // Fix: Convert shipping_method and shipping_option from array to string if needed
        if (is_array($data['shipping_method'])) {
            $data['shipping_method'] = implode(',', $data['shipping_method']);
        }
        if (is_array($data['shipping_option'])) {
            $data['shipping_option'] = implode(',', $data['shipping_option']);
        }
    ...
    public function createOrder(Request $request, int|string $currentUserId, string $token, array $cartItems)
    {
        ...
        if (isset($input['shipping_option']) && is_array($input['shipping_option'])) {
            $input['shipping_option'] = implode(',', $input['shipping_option']);
        }
