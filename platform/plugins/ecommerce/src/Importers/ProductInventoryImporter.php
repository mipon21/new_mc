...
class ProductInventoryImporter extends Importer
{
    ...
    public function columns(): array
    {
        return [
            ...
            ImportColumn::make('shipping_weeks')
                ->rules(
                    ['required_if:stock_status,in_stock_with_shipping', 'string'],
                    trans('plugins/ecommerce::product-inventory.import.rules.shipping_weeks')
                ),
        ];
    }
    ...
    public function examples(): array
    {
        $products = Product::query()
            ->take(5)
            ->wherePublished()
            ->select(['id', 'name', 'sku', 'with_storehouse_management', 'quantity', 'stock_status', 'shipping_weeks'])
            ->get();
        ...
        if ($products->isNotEmpty()) {
            return $products->transform(function ($product) {
                return [
                    ...
                    'shipping_weeks' => $product->shipping_weeks,
                ];
            })->all();
        }
        ...
        return [
            ...
            [
                ...
                'quantity' => 23,
                'stock_status' => 'in_stock_with_shipping',
                'shipping_weeks' => '2-4',
            ],
        ];
    }
