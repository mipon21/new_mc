...
class HandleCheckoutOrderData
{
    ...
    public function execute(Request $request, Collection $products, string $token, array &$sessionCheckoutData): CheckoutOrderData
    {
        ...
        // Calculate shipping tax amount but don't add it to order amount since it's already included in rawTotal
        $shippingTaxAmount = Cart::instance('cart')->rawShippingTax($shippingAmount);
